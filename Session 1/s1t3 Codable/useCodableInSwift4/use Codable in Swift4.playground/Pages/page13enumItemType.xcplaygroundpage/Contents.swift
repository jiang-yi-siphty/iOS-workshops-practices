...
struct HomepageModule: Codable {
  ...
  init(from decoder: Decoder) throws {
    ...
    //    moduleName = try container.decode(ModuleName.self, forKey: .moduleName)
    visible = try container.decode(Bool.self, forKey: .visible)
    items = try container.decodeIfPresent([Item].self, forKey: .items)  //🍓🥕 Look at here, for optional var decoder, we need to use decodeIfPresent()
  }
